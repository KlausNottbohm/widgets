...
async function run() {
    ...
    async function createWidget() {
        ...
        let { fresh, myStoredData, wifiProblem } = await getStoredData(fm, path);
        if (wifiProblem) {
        ...
        showObject(myStoredData, "myResult");
        ...
        showTitle(widget, fresh);
        showStoredData(myDrawContext, myStoredData);
...
function showTitle(widget, fresh) {
...
function showStoredData(pDrawContext, pStoredData) {
    ...
    pDrawContext.setFont(Font.mediumSystemFont(26));
    pDrawContext.setTextColor(myTextColor);
    pDrawContext.drawTextInRect(pStoredData.data.usedVolumeStr + " / " + pStoredData.data.initialVolumeStr, myRestDataRect);
    ...
    pDrawContext.drawTextInRect(myDateString, myEndDateRect);
    ...
    let myWidth = 220;
    pDrawContext.setFont(Font.mediumSystemFont(22));
    let myAppInfoRect = new Rect(bedsPaddingLeft, bedsGraphBaseline - 0, widgetWidth - myWidth, 26);
    pDrawContext.drawTextInRect(myServerTime, myAppInfoRect);

    let myVersionInfoRect = new Rect(bedsPaddingLeft + widgetWidth - myWidth, bedsGraphBaseline - 0, myWidth, 26);
    pDrawContext.setFont(Font.italicSystemFont(20));
    pDrawContext.drawTextInRect(conVersion, myVersionInfoRect);
    return pDrawContext;
}
 ...
 * @param {string} path
 */
async function getStoredData(fm, path) {
    // ServerData
